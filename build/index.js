...
server.tool("save_file_as", "Save the current Frame0 document with a new filename to the muzika/wireframes directory.", {
...
server.tool("save_file_to_path", "Save the current Frame0 document to a specific file path (automatic save without dialog).", {
    ...
    try {
        ...
        console.error(error);
        return response.error(JsonRpcErrorCode.InternalError, `Failed to save file to path: ${error instanceof Error ? error.message : String(error)}`);
    }
});
server.tool("save_file_to_muzika_wireframes", "Save the current Frame0 document to the muzika/wireframes directory with a timestamp.", {}, async () => {
    try {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        ...
        console.error(error);
        return response.error(JsonRpcErrorCode.InternalError, `Failed to save file to muzika wireframes: ${error instanceof Error ? error.message : String(error)}`);
    }
});
async function main() {
